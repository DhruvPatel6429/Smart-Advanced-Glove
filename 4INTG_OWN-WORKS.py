## WORKINGGG!!! DAMMN IT! FINAL CONNECTED ! 
# (only the flex sensors are very deflecting, showing random values for few fingers)
...
class ArduinoReader(threading.Thread):
    def __init__(self, port='COM4', baudrate=9600):
        ...
        self.angles = [0, 0, 0, 0]  # default to 0
        self.temp_angles = []
        self.daemon = True
        self.start()
        ...
        while True:
            try:
                ...
                if line:
                    ...
                    # Match your output: "[SERIAL] 1: 471  Angle : 29°"
                    if "Angle" in line:
                        ...
                        if len(parts) == 2:
                            ...
                            # If we have 4, save as latest snapshot
...
#         # ✅ Correct flex sensor index:
#         if finger == "Thumb":
#             cv2.putText(dashboard, "Flex: N/C", (x, 210),
#                         cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
#         else:
#             ard_idx = idx - 1  # Index for Arduino: Index=0, Middle=1, Ring=2, Pinky=3
#             if arduino_angles and ard_idx < len(arduino_angles) and arduino_angles[ard_idx] is not None:
#                 cv2.putText(dashboard, f"Flex: {arduino_angles[ard_idx]}°", (x, 210),
#                             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
#             else:
#                 cv2.putText(dashboard, "Flex: N/C", (x, 210),
#                             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
...
def create_dashboard(angle_dict, arduino_angles, anomaly_dict, arduino_anomaly, fps, frame_width):
    ...
    for idx, finger in enumerate(fingers):
        ...
        # ✅ Proper flex value print
        if finger != "Thumb":
            flex_idx = idx - 1  # because Thumb has no flex
            if flex_idx < len(arduino_angles):
...
def main():
    ...
    while True:
        ...
        arduino_angles = arduino.angles.copy()
        arduino_anomaly = []
        ...
        for idx, finger in enumerate(fingers):
            if finger == "Thumb":
                ...
                flex_idx = idx - 1
                double_anomaly[finger] = anomalies[finger] and arduino_anomaly[flex_idx]
