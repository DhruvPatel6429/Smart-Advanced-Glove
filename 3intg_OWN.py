# Working. only serial port values, ie angles are shown in terminal!!
# SERIAL PORT ANGLE VALUES IN TERMINAL WORKING! 
# DASHBOARD NOT POPPING UP.
...
# --- ARDUINO SERIAL READER THREAD ---
# class ArduinoReader(threading.Thread):
#     def __init__(self, port='COM4', baudrate=9600):
#         super().__init__()
#         self.port = port
#         self.baudrate = baudrate
#         self.angles = [None, None, None, None]  # Flex sensor angles
#         self.running = True
...
#                     vals = line.replace("FLEX:", "").split(",")
#                     if len(vals) == 4:
#                         try:
#                             self.angles = [int(float(v)) for v in vals]
#                         except ValueError:
#                          # If bad data comes, keep old angles
#                             continue
#             except Exception:
#                 continue
...
#         # Arduino readings (with check)
#         if idx < len(arduino_angles):
#             ard_angle = arduino_angles[idx]
#             if ard_angle is None:
#                 flex_text = "Flex:N/C"
#             else:
#                 flex_text = f"Flex:{int(ard_angle)}\u00B0"
#             ard_color = (0, 0, 255) if arduino_anomaly[idx] else (0, 255, 255)
#             cv2.putText(dashboard, flex_text, (x, 210),
...
#     spacing = 20
#     start_x = 20
#     fingers = list(angle_dict.keys())  # ["Thumb", "Index", "Middle", "Ring", "Pinky"]
...
#         # Flex sensor display logic:
#         if finger == "Thumb":
#             flex_text = "Flex:N/C"
#             ard_color = (50, 50, 50)
#         else:
#             ard_idx = idx - 1  # Shift by 1 because Thumb has no flex
#             if ard_idx < len(arduino_angles):
#                 ard_angle = arduino_angles[ard_idx]
#                 if ard_angle is not None and ard_angle > 0:
#                     flex_text = f"Flex:{int(ard_angle)}°"
#                 else:
#                     flex_text = "Flex:N/C"
#                 ard_color = (0, 0, 255) if arduino_anomaly[ard_idx] else (0, 255, 255)
#             else:
#                 flex_text = "Flex:N/C"
#                 ard_color = (50, 50, 50)
...
def create_dashboard(angle_dict, arduino_angles, anomaly_dict, arduino_anomaly, fps, frame_width):
    ...
    for idx, finger in enumerate(fingers):
        ...
        cv2.putText(dashboard, f"{int(angle)}°", (x + 5, 190),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        # ✅ Flex value print here
        if idx < len(arduino_angles):
            flex_val = arduino_angles[idx]
...
# def main():
#     arduino = ArduinoReader(port='COM4', baudrate=9600)  # Change port if needed
...
#     frame_count, fps_counter, fps = 0, 0, 0
#     fps_timer = time.time()
...
#             else:
#                 arduino_anomaly.append(not (angle < 30 or angle > 150))
...
def main():
    arduino = ArduinoReader(port='COM4', baudrate=9600)  # Adjust your COM port if needed
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        print("Error: Could not open video stream.")
        return
    ...
    while True:
        ...
        # --- Arduino Data ---
        arduino_angles = arduino.angles  # This directly gives the latest angles
        arduino_anomaly = []
        for angle in arduino_angles:
        ...
        # Combined anomaly only for fingers with Arduino flex:
        double_anomaly = {}
        for idx, finger in enumerate(fingers):
            if finger == "Thumb":
                double_anomaly[finger] = anomalies[finger]  # Thumb: no Arduino
            else:
                ard_idx = idx - 1
                double_anomaly[finger] = anomalies[finger] and arduino_anomaly[ard_idx]
